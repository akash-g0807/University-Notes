...
We {\em can say} that each of the $\underline{e_a}^{'}$ is a {\bf linear combination} of each if the $\underline{e_a}$. So we can say that for a {\bf matrix} $R_{ab}$
\begin{equation}
	\underline{e_{a}^{'}}= R_{ab} \ \underline{e_{b}}\tag{$*$}\label{eq: lin-comb}
\end{equation}
Since the index $b$ is {\bf repeated twice}, we use the {\bf Einstein Convention}. This is equivalent to:
$$\underline{e_{a}^{'}}= R_{a1}\ \underline{e_{1}} \ +\  R_{a2}\ \underline{e_{2}} \ +\  R_{a3}\ \underline{e_{3}}$$
We {\bf need to work out} that $R_{ab}$ is.


First we {\em require} from the definition of Kronecker Delta \ref{eq:kronecker-delta}:
...
and hence $R$ is an {\bf orthogonal} matrix. Also note that
